...
while(opcionElegida != "-1"):
  print("\nBienvenido al generador de arboles sintacticos abstractos de MiniJava")
  ...
  print("\nOprima -1 para salir del programa\n")
  ...
  if(opcionElegida in test):
    ...
    nodoRaiz = parser.parse(codigoFuente)
    
    codigo_DOT = nodoRaiz.generarCodigo_DOT()
    file = open('codigo_DOT.txt', 'w+')
    file.write(codigo_DOT)
    file.close()
    print("\nEl codigo DOT se ha generado exitosamente!"\
          "\nSe ha creado el archivo codigo_DOT.txt en la carpeta actual")
